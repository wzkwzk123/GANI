...
def mlp(x, hidden_sizes, output_dim, scope):
    with tf.variable_scope(scope):
        ...
        out_put3 = tf.layers.dense(out_put2, units=30, activation=tf.nn.relu, name="third_layer")
        out_put4 = tf.layers.dense(out_put3, units=output_dim, activation=tf.nn.sigmoid, name="output_layer")
    return out_put4
